...
const fillIcons: Record<string, string> = {
  stop: '<rect x="5.5" y="5.5" width="13" height="13" rx="3"></rect>',
}

const isFill = (n: string) => n in fillIcons
</script>

<template>
  <svg
    v-if="isFill(props.name)"
    :width="props.size"
    :height="props.size"
    viewBox="0 0 24 24"
    fill="currentColor"
    stroke="none"
    v-html="fillIcons[props.name]"
  />
  <svg
    v-else
    :width="props.size"
    ...
    v-html="icons[props.name] ?? icons.activity"
  />
</template>
